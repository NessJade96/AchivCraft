...
router.get("/achievement", function (req, res) {
	return __awaiter(this, void 0, void 0, function* () {
		...
		const refetchedAchievementPromises = getOldAchievementsData.map(
			(character) =>
				__awaiter(this, void 0, void 0, function* () {
					...
					const latestAchievementsWithTimestamp =
						latestAchievements.filter(
							// @ts-expect-error get deployment working
							(achievement) => achievement.completed_timestamp
						);
					// @ts-expect-error get deployment working
					return latestAchievementsWithTimestamp.map(
						(achievement) => {
							return {
		...
		const refetchedAchievements = yield Promise.all(
		...
		let {
			data: characterAchievementData,
			error: characterAchievementError,
		} = yield supabase
			.from("achievement")
			.select(
				`
